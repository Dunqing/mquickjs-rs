...
pub const TYPED_ARRAY_OBJECT_MARKER: i32 = 1 << 18;
...
impl Value {
    ...
    /// Create a TypedArray object Value from an index
    #[inline]
    pub const fn typed_array_object(idx: u32) -> Self {
        Value(RawValue::make_special(
            SpecialTag::CatchOffset as u8,
            (idx as i32) | TYPED_ARRAY_OBJECT_MARKER,
        ))
    }

    /// Check if this is a TypedArray object
    #[inline]
    pub const fn is_typed_array_object(self) -> bool {
        self.0.get_special_tag() == SpecialTag::CatchOffset as u8
            && (self.0.get_special_value() & TYPED_ARRAY_OBJECT_MARKER) != 0
            && (self.0.get_special_value() & (ERROR_OBJECT_MARKER | REGEXP_OBJECT_MARKER)) == 0
    }

    /// Get TypedArray object index, returns None if not a TypedArray object
    #[inline]
    pub const fn to_typed_array_object_idx(self) -> Option<u32> {
        if self.is_typed_array_object() {
            Some((self.0.get_special_value() & !TYPED_ARRAY_OBJECT_MARKER) as u32)
        } else {
            None
        }
    }
